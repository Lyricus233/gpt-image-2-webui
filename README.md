<div align="center">
  ...
  <p>
    ...
    <img src="https://img.shields.io/badge/Models-gpt--image--2_%7C_gpt--image--2--pro-18A978?style=flat-square" alt="Model options gpt-image-2 and gpt-image-2-pro" />
...
GPT Image Playground 是一个面向 `gpt-image-2` 的单页图片生成与编辑工作台。它通过 `OPENAI_API_BASE_URL` 配置 OpenAI 兼容接口，默认模型选项包含 `gpt-image-2` 和 `gpt-image-2-pro`，也支持输入自定义模型 ID。API Key 可直接在首页顶部填写。
...
- 模型选择：默认提供 `gpt-image-2` 和 `gpt-image-2-pro`，也支持输入自定义模型 ID。
- 首页 API Key：可在首页顶部填写，自动保存到当前浏览器 localStorage，并提供获取 Key 的快捷入口。
- 接口地址：通过 `OPENAI_API_BASE_URL` 配置 OpenAI 兼容接口。
...
- 额度价值对比：按本站 `¥1 = 10 张图` 计算单图 `¥0.1`，对比官方估算成本，展示大约节省金额和划算倍数。
...
- 本站额度价：按 `¥1 = 10 张图` 计算，帮助用户直观看到额度价值。
...
```text
lyricus/gpt-image-2-webui:latest
```

推荐使用 Docker Compose 部署，默认会直接拉取 Docker Hub 镜像。仓库内 [docker-compose.yml](docker-compose.yml) 默认仅将端口绑定到宿主机回环地址 `127.0.0.1:3000`，并接入名为 `ai-net` 的外部网络，需先创建该网络再启动：
...
在默认 `fs` 存储模式下，生成图片会持久保存在宿主机的 `generated-images` 目录。

### 反向代理

服务默认仅监听 `127.0.0.1:3000`，无法通过公网 IP 直接访问，需由同机的反向代理（Nginx、Caddy、Traefik 等）将外部请求转发至 `127.0.0.1:3000`。如反向代理同样部署在 Docker 中，可将代理容器加入 `ai-net` 网络，通过容器名 `gpt-image-2-webui:3000` 直接访问，无需占用宿主机回环端口。如不需要反向代理，可将 [docker-compose.yml](docker-compose.yml) 中端口映射改为 `"${PORT:-3000}:3000"` 直接对外暴露。
...
容器内应用以非 root 的 `nextjs` 用户运行。若宿主机的 `./generated-images` 由 root 拥有，容器进程将无法写入，运行时会出现如下错误：

```text
EACCES: permission denied, open '/app/generated-images/xxx.png'
```

在 [docker-compose.yml](docker-compose.yml) 所在目录执行以下命令完成赋权（需先启动一次容器以便读取容器内 UID/GID）：

```bash
mkdir -p generated-images
APP_UID=$(docker exec gpt-image-2-webui id -u)
APP_GID=$(docker exec gpt-image-2-webui id -g)
sudo chown -R "$APP_UID:$APP_GID" generated-images
sudo chmod -R u+rwX,g+rwX generated-images
docker restart gpt-image-2-webui
```

镜像中 `nextjs` 用户的 UID 通常为 `1000`，也可在首次启动前直接 `sudo chown -R 1000:1000 generated-images`，再以容器内 `id` 输出为准核对。
...
也可以不克隆源码，直接使用 `docker run`（建议同样绑定到 `127.0.0.1` 并经反向代理转发，挂载目录赋权方式参见"挂载目录权限"小节）：

```bash
docker run -d --name gpt-image-2-webui --restart unless-stopped -p 127.0.0.1:3000:3000 -e OPENAI_API_BASE_URL=https://your-openai-compatible-api.example/v1 -e NEXT_PUBLIC_IMAGE_STORAGE_MODE=fs -v ${PWD}/generated-images:/app/generated-images lyricus/gpt-image-2-webui:latest
```
...
你可以直接在首页顶部填写 API Key，也可以点击“获取 Key”在新标签页打开由 `OPENAI_API_BASE_URL` 推导出的 `/console/token` 创建令牌。Base URL 从 `OPENAI_API_BASE_URL` 读取；也可以使用环境变量配置服务端默认 API Key。
...
服务端请求使用 `OPENAI_API_BASE_URL`。未配置时，请求会返回配置错误：
...
Docker Hub 镜像 `lyricus/gpt-image-2-webui:latest` 默认按 `fs` 模式构建；如果需要 Docker 环境使用 `indexeddb`，请按上面的源码构建命令重新构建镜像。
